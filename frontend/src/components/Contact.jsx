...
function ContactCard({ icon, title, children, delay = 0 }) {
  ...
  return (
    <div 
      ref={ref}
      className={`relative bg-gradient-to-br from-gray-900 to-black border-2 border-pink-500/30 rounded-xl p-6 hover:border-pink-500 transition-all hover:shadow-lg hover:shadow-pink-500/30 group overflow-hidden scroll-scale ${isVisible ? 'visible' : ''}`}
...
function Contact() {
  ...
  const [socialRef, socialVisible] = useScrollAnimation({ threshold: 0.2 })
  ...
  return (
    <section id="contact" className="bg-black text-white py-20 px-4 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        {/* Заголовок */}
        <div ref={titleRef} className={`text-center mb-12 md:mb-16 px-4 scroll-fade-up ${titleVisible ? 'visible' : ''}`}>
          <h2 className="font-horror text-4xl sm:text-5xl md:text-6xl lg:text-8xl uppercase tracking-[0.15em] text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-pink-400 to-pink-600 mb-4 py-2">
            Pink Ink Love
          </h2>
          <div className="w-24 md:w-32 h-1 bg-gradient-to-r from-transparent via-pink-500 to-transparent mx-auto"></div>
        </div>

        {/* Контактные карточки */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12 px-4">
          {/* Адрес */}
          <ContactCard
            delay={0} 
            icon={
        ...
        {/* Социальные сети */}
        <div ref={socialRef} className={`flex flex-wrap justify-center gap-3 md:gap-4 px-4 scroll-fade-up ${socialVisible ? 'visible' : ''}`} style={{ transitionDelay: '600ms' }}>
          {/* Telegram */}
          <a 
            href="https://t.me/plo_hishka" 
            target="_blank" 
            rel="noopener noreferrer"
            className="group relative inline-flex items-center gap-2 md:gap-3 bg-gradient-to-r from-pink-600 via-pink-500 to-pink-600 bg-[length:200%_100%] animate-shimmer text-black font-bold px-4 md:px-8 py-3 md:py-4 rounded-xl overflow-hidden transition-all hover:scale-105 shadow-2xl shadow-pink-500/50 hover:shadow-pink-500/70 text-sm md:text-base"
          >
          ...
          >
            ...
            <span className="uppercase tracking-wider">VK</span>
          </a>
        </div>
      </div>
    </section>
